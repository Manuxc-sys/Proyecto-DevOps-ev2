...
// 💡 1. CONFIGURAMOS LAS VARIABLES DE ENTORNO AQUÍ AFUERA
// ¿Cómo funciona esto? El operador "||" significa: "Si GitHub Actions inyectó una IP de AWS, úsala. Si no encuentra nada (porque estás en tu PC local), usa localhost por defecto". ¡Así no se rompe en ningún lado!
const API_VENTAS_URL = import.meta.env.VITE_API_VENTAS_URL || "http://localhost:8081";
const API_DESPACHOS_URL = import.meta.env.VITE_API_DESPACHOS_URL || "http://localhost:8082";
...
export const FormDespacho = ({ venta, onClose }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      // 💡 2. REEMPLAZAMOS LAS URLS FIJAS POR LAS VARIABLES DINÁMICAS
      // Usamos comillas invertidas (``) para concatenar la variable con la ruta del endpoint
      await axios.put(
